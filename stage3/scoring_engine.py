...
class PhotoScorer:
    ...
    def score_photo(self, image_path, people_in_photo_df):
        ...
        category_metrics = self._calculate_all_person_metrics(gray_image, people_in_photo_df)
    ...
    def _calculate_single_person_metrics(self, gray_image, facial_area):
        ...
        eye_score = 1 if ear > thresholds['eye_aspect_ratio'] else 0
        smile_score = 1 if smile > thresholds['smile_ratio'] else 0
        
        face_crop = gray_image[y1:y2, x1:x2]
        focus_score = 1 if self._is_sharp(face_crop, thresholds['face_focus']) else 0
    ...
    def _is_sharp(self, image_gray, threshold):
        if image_gray is None or image_gray.size == 0: return False
        return cv2.Laplacian(image_gray, cv2.CV_64F).var() > threshold
